...
def generate_hpt_filename(mode, dtime, totaltime, fecha, path, calendar, file_extension):
    ...
    if mode == "backward":
        ...
        for i in array:
            ...
            name = path + "particles_output_" + fecha_dia+ "_"+ fecha_hour+"."+file_extension
...
def checking_raw_hpt_partposti_files(partpositfiles, listdates, file_extension):
    errors = ""
    find_error = False
...
def get_parcels_over_target_region(variables,
    name_file, file_mask, maskname, maskvar_lon, maskvar_lat, mask_value,fdate,file_extension
):
    """
    ...
    if file_extension=="csv":
        ds = pd.read_csv(name_file)
        # Load directly into numpy arrays.

        #ds_t = ds.sel(time=ts_fdate)


        part_id = ds["id"].values.flatten()
        part_lon = ds["longitude"].values.flatten()
        part_lat = ds["latitude"].values.flatten()
        part_pres = ds["pressure"].values.flatten()
    elif file_extension=="nc":
        ...
        with xr.open_dataset(name_file, chunks={}) as ds:
            ...
            part_id = ds["particle_id"].values.flatten()
            part_lon = ds["longitude"].values.flatten()
            part_lat = ds["latitude"].values.flatten()
            part_pres = ds["pressure"].values.flatten()
...
def read_by_proccesor(
    ...
    for i in a1:
        ...
        matrix_i = read_file(partpositfiles[i],
                        parcel_ids,
                        local_dates[i],
                        lon_left_lower_corner,
                        lat_left_lower_corner,
...
def read_file(name_file,
    ...
    if file_extension=="csv":
        ds_subset = pd.read_csv(name_file)

        # Use .isin() to filter based on a list of items
        target_ds = ds_subset[ds_subset["id"].isin(parcel_ids)]

        # drop=True gets rid of the old index entirely
        target_ds = target_ds.reset_index(drop=True)
    elif file_extension=="nc":
        ds_subset = xr.open_dataset(name_file, chunks={})
        target_ds = ds_subset.sel(particle_id=parcel_ids).load()
    variables = ["latitude", "longitude", "temperature", "pressure","specific_humidity"]
...
def get_vars_from_hpt(
    ...
    parcel_ids=get_parcels_over_target_region( variables=['longitude','latitude'], name_file=name_file, file_mask=file_mask, maskname=maskname, maskvar_lon=maskvar_lon, maskvar_lat=maskvar_lat, mask_value=mask_value,fdate=fdate, file_extension=file_extension)
